...
public class Shimmer {
  ...
  public Shimmer setDirection(int direction) {
    ...
    if (direction != ANIMATION_DIRECTION_LTR && direction != ANIMATION_DIRECTION_RTL) {
      throw new IllegalArgumentException(
          "The animation direction must be either ANIMATION_DIRECTION_LTR or ANIMATION_DIRECTION_RTL");
    }
  ...
  public <V extends View & ShimmerViewBase> void start(final V shimmerView) {
    ...
    final Runnable animate =
        new Runnable() {
          ...
          public void run() {
            ...
            animator =
                android.animation.ObjectAnimator.ofFloat(shimmerView, "gradientX", fromX, toX);
    ...
    if (!shimmerView.isSetUp()) {
      shimmerView.setAnimationSetupCallback(
          new ShimmerViewHelper.AnimationSetupCallback() {
            @Override
